...
import { Card, Empty, Button, Row, Col, Layout, message, Select, Pagination } from 'antd';
...
const CoursesEnrollment = () => {
  ...
  const { enrolledCourses, setEnrolledCourses  } = useContext(SignInContext);
  ...
  const enrollCourseMutation = useMutation({
    mutationFn: async (courseId) => {
      const response = await fetch(`http://localhost:8080/user/enrollcourse/${courseId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (!response.ok) {
        ...
        message.error(errorText || "Failed to enroll in course");
        throw new Error(errorText || "Failed to enroll in course");
      }
      return courseId;
    },
    onSuccess: (courseId) => {
      message.success("Enrolled in course successfully!");
      setEnrolledCourses([...enrolledCourses, courseId.toString()]); 
    },  
  });
  ...
  const leaveCourseMutation = useMutation({
    mutationFn: async (courseId) => {
      const response = await fetch(`http://localhost:8080/user/leavecourse/${courseId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        message.error(errorText || "Failed to leave course");
        throw new Error(errorText || "Failed to leave course");
      }

      message.success("Left course successfully!");
      setEnrolledCourses(enrolledCourses.filter(id => id !== courseId.toString())); 
    },
  });
            ...
            paginatedCourses.map((course) => (
              <Col key={course.courseId} xs={24} sm={12} md={8}>
                <CourseCard
                  ...
                  extra={
                    enrolledCourses.includes(course.courseId.toString()) ? (
                        <Button type="primary" danger onClick={() => handleLeaveCourse(course.courseId)}>
                          Leave Course
                        </Button>
                        ) : (
                        <Button type="primary" onClick={() => handleEnrollCourse(course.courseId)} disabled={enrolledCoursesCount>=3}>
                          Enroll
                        </Button>
                        )
                    }
